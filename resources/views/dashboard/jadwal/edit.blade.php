...
@section('container')
    <h2 class="mb-4">Edit Jadwal Peserta</h2>
    <hr class="my-4">
            ...
            <div class="card">
                <div class="card-body">
                    <div class="card-title">
                        <a href="/dashboard/jadwal-peserta" class="btn mb-1 btn-secondary"><i class="bi bi-arrow-left"></i>
                            Kembali</a>
                    </div>
                    <div class="form-validation">
                        <form class="form-valide" action="/dashboard/jadwal-peserta/{{ $jadwal->id }}" method="post">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-lg-6">
                                    <select class="form-control @error('nism') is-invalid @enderror" id="nism"
                                        name="nism">
                                        <option value="" disabled selected>Pilih NIS / NIM</option>
                                        @foreach ($details as $detail)
                                            <option value="{{ $detail->id }}" data-nama="{{ $detail->nama_peserta }} "
                                                @if (old('nism', $jadwal->detailMagang->id) == $detail->id) selected @endif>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-lg-6">
                                    <input type="text" class="form-control" id="nama_peserta" name="nama_peserta"
                                        value="{{ old('nama_peserta', $jadwal->detailMagang->nama_peserta) }}" readonly>
                                </div>
                            </div>
                            ...
                            <div class="form-group row">
                                <label for="nama_pegawai" class="col-lg-4 col-form-label">Nama Kepala Ruangan /
                                    Staff</label>
                                <div class="col-lg-6">
                                    <select class="form-control @error('ruangan_id') is-invalid @enderror" id="nama_pegawai"
                                        name="ruangan_id">
                                        ...
                                        @foreach ($ruangans as $ruangan)
                                            <option value="{{ $ruangan->id }}" data-ruangan="{{ $ruangan->nama_ruangan }}"
                                                data-kepala="{{ $ruangan->nama_kepala_ruangan }}"
                                                @if (old('ruangan_id', $jadwal->ruangan_id) == $ruangan->id) selected @endif>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-lg-6">
                                    <input type="text" class="form-control" id="nama_ruangan" name="nama_ruangan"
                                        value="{{ old('nama_ruangan', $jadwal->ruangan->nama_ruangan) }}" readonly>
                                </div>
                            </div>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-lg-6">
                                    <input type="date"
                                        ...
                                        value="{{ old('tanggal_awal', $jadwal->tanggal_awal) }}">
                                    @error('tanggal_awal')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Jika sudah ada pilihan yang disimpan (seperti pada edit), update nama ruangan
        const selectedOption = kepalaRuanganDropdown.options[kepalaRuanganDropdown.selectedIndex];
        namaRuanganInput.value = selectedOption ? selectedOption.getAttribute('data-ruangan') : '';
